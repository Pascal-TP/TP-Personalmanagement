...
function vacationBalanceHtml(rows,year){
  ...
  return `<div class="vacation-balance-sheet print-sheet"><div class="birthday-title"><div><small>TP-Personalmanagement</small><h2>Resturlaub ${year}</h2><p>Resturlaub aus dem Vorjahr wird zuerst verbraucht und ist grundsätzlich bis 31.03. zu nehmen. Eine Fristverlängerung ist nur mit Begründung der Führungskraft und Zustimmung der Personalabteilung möglich.</p></div><span>Stand ${deDate(new Date())}</span></div>
    ...
    <div class="table-wrap"><table><thead><tr><th>Mitarbeiter</th><th>Firma</th><th>Anspruch</th><th>Übertrag</th><th>davon genutzt</th><th>verfallen</th><th>laufendes Jahr genutzt</th><th>verfügbar</th><th>Frist / Ausnahme</th></tr></thead><tbody>
      ${rows.length?rows.map(r=>`<tr><td><strong>${esc(r.name)}</strong></td><td>${esc(r.company)}</td><td>${r.entitlement}</td><td>${r.carryover}</td><td>${r.carryoverUsed}</td><td>${r.expired}</td><td>${r.currentUsed}</td><td><strong class="${r.totalRemaining<0?'negative-text':''}">${r.totalRemaining}</strong></td><td>${r.extensionUntil?`bis ${esc(r.extensionUntil.split('-').reverse().join('.'))}<br><small>${esc(r.extensionReason)}</small>`:`31.03.${year}`}</td></tr>`).join(''):'<tr><td colspan="9" class="empty">Keine Mitarbeiter vorhanden.</td></tr>'}
