...
async function renderSupervisorAttendance(el,ctx){
  ...
  el.querySelector('#supervisor-annual-show').onclick=async()=>{const user=users.find(u=>u.id===form.elements.userId.value),year=Number(form.elements.year.value);if(!user||year<2020||year>2100){toast('Bitte Mitarbeiter und gültiges Jahr auswählen.','error');return}let settings=[];try{const snap=await getDocs(query(collection(db,'vacationCarryoverSettings'),where('userId','==',user.id)));settings=snap.docs.map(d=>({id:d.id,...d.data()}))}catch(e){console.warn(e)}result.innerHTML=annualHtml(user,year,vacations,absences,settings)};
  renderWeek();
}
