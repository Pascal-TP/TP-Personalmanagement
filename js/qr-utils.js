...
export async function printPersonnelQr(token){
  ...
  w.document.open();w.document.write(`<!doctype html><html lang="de"><head><meta charset="utf-8"><title>QR-Code Zeiterfassung</title><style>@page{size:auto;margin:10mm}html,body{margin:0;background:#fff}.cut-frame{width:17.65mm;height:17.65mm;box-sizing:border-box;border:.2mm solid #555;display:grid;place-items:center;break-inside:avoid}.code{width:17.25mm;height:17.25mm;display:block;image-rendering:pixelated}</style></head><body><div class="cut-frame"><img class="code" src="${dataUrl}" alt="QR-Code"></div><script>window.onload=()=>setTimeout(()=>window.print(),100)<\/script></body></html>`);w.document.close();
}
