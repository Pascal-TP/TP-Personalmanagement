...
import { esc, fmtDate, toast, confirmDialog } from "./utils.js";
...
export async function renderNews(el,ctx){
  ...
  el.querySelectorAll('.delete-news').forEach(b=>b.onclick=async()=>{if(await confirmDialog('Hinweis wirklich löschen?')){await deleteDoc(doc(db,'news',b.dataset.id));renderNews(el,ctx)}})
}
