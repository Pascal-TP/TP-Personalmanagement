...
import { esc, fmtDate, fmtDateTime, statusPill, toast, confirmDialog } from "./utils.js";
...
function portionLabel(v){return v?.dayPortion==='morning'?' · ½ Tag vormittags':v?.dayPortion==='afternoon'?' · ½ Tag nachmittags':''}
function requestedDays(from,to,workDays,dayPortion,type='Urlaub'){const n=workdays(from,to,workDays);return type==='Urlaub'&&from===to&&['morning','afternoon'].includes(dayPortion)?n*0.5:n}
...
export async function renderUrlaub(el,ctx){
  ...
  const hrBlock=canManageAbsences?`<article class="card"><div class="card-head"><div><h2>Abwesenheit buchen</h2><p>Urlaub, Krankheit und weitere Abwesenheiten direkt durch die Personalabteilung erfassen. Urlaub gilt sofort und benötigt keine zusätzliche Freigabe.</p></div></div>
    <form id="absence-form" class="form-grid">
      ...
      <label class="field"><span>Umfang Urlaub</span><select name="dayPortion"><option value="full">Ganzer Tag</option><option value="morning">½ Tag vormittags</option><option value="afternoon">½ Tag nachmittags</option></select></label><label class="field"><span>Bemerkung</span><input name="note" placeholder="optional"></label>
  ...
  el.innerHTML=`<div class="kpi-grid three ${isAdmin?"admin-self-hidden":""}"><div class="kpi"><span>Anspruch</span><strong>${ctx.profile.vacationDays||30}</strong><small>Urlaubstage</small></div><div class="kpi"><span>Genehmigt</span><strong>${approved}</strong><small>Urlaubstage</small></div><div class="kpi"><span>${canManageAbsences?'Eigene Krankheit':'Beantragt'}</span><strong>${canManageAbsences?sick:pending}</strong><small>Tage</small></div></div>${vacBalance&&!isAdmin?`<div class="info-strip"><strong>Resturlaub ${vacationYear}:</strong> ${vacBalance.carryover} Tage aus ${vacationYear-1} übertragen · ${vacBalance.carryoverUsed} Tage davon bereits genutzt · ${vacBalance.carryoverRemaining} Tage noch verfügbar${vacBalance.expired?` · ${vacBalance.expired} Tage verfallen`:''}. Frist: ${fmtDate(vacBalance.expiry)}. Genehmigter Urlaub bis zur Frist wird automatisch zuerst auf den Resturlaub angerechnet.</div>`:''}
    ...
    ${canApprove?`<article class="card"><div class="card-head"><div><h2>Urlaubsfreigaben</h2><p>Offene Anträge der zugeordneten Mitarbeiter sowie zurückgezogene Anträge zur Kenntnisnahme.</p></div></div><div id="approval-list">
      ${withdrawnTeam.length?`<div class="info-strip"><strong>${withdrawnTeam.length} zurückgezogene${withdrawnTeam.length===1?'r':''} Antrag${withdrawnTeam.length===1?'':'e'}</strong> wartet/warten auf Kenntnisnahme.</div>${withdrawnTeam.map(v=>`<div class="approval-row withdrawn-vacation-row"><div><strong>${esc(v.userName||v.userId)}</strong><span>${fmtDate(v.from)} – ${fmtDate(v.to)} · ${v.days||0} Tage${portionLabel(v)} · zurückgezogen</span>${vacationHistory(v)}</div><div class="actions"><button class="btn small secondary acknowledge-withdrawal" data-id="${v.id}">Zur Kenntnis genommen</button></div></div>`).join('')}`:''}
      ${pendingTeam.length?pendingTeam.map(v=>`<div class="approval-row"><div><strong>${esc(v.userName||v.userId)}</strong><span>${fmtDate(v.from)} – ${fmtDate(v.to)} · ${v.days||0} Tage${portionLabel(v)}</span>${v.note?`<small>${esc(v.note)}</small>`:''}</div><div class="actions"><button class="btn small approve" data-id="${v.id}">Genehmigen</button><button class="btn small danger reject" data-id="${v.id}">Ablehnen</button></div></div>`).join(""):(withdrawnTeam.length?'':'<div class="empty">Keine offenen Freigaben.</div>')}
    </div></article>`:''}`;

  const vacForm=el.querySelector('#vac-form');if(vacForm)vacForm.onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),d=Object.fromEntries(f.entries());if(d.to<d.from){toast("Der Bis-Termin liegt vor dem Von-Termin.");return}if(d.dayPortion!=="full"&&(d.type!=="Urlaub"||d.from!==d.to)){toast("Ein halber Tag ist nur bei einem eintägigen Urlaubsantrag möglich.","error");return}const ownSupervisors=supervisorIdsOf(ctx.profile);if(!ownSupervisors.length){toast('Für diesen Benutzer ist kein Vorgesetzter hinterlegt. Der Antrag kann derzeit nicht weitergeleitet werden.','error');return}await addDoc(collection(db,"vacationRequests"),{...d,days:requestedDays(d.from,d.to,ctx.profile.workDays,d.dayPortion,d.type),userId:ctx.profile.id,userName:ctx.profile.name||ctx.profile.email,companyId:ctx.profile.companyId,supervisorId:ctx.profile.supervisorId||null,supervisorId2:ctx.profile.supervisorId2||null,status:"pending",createdAt:serverTimestamp()});toast("Urlaubsantrag gesendet.");renderUrlaub(el,ctx)};
  ...
  const af=el.querySelector('#absence-form');if(af)af.onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,user=employees.find(u=>u.id===f.elements.userId.value);if(!user)return;if(f.elements.to.value<f.elements.from.value){toast('Der Bis-Termin liegt vor dem Von-Termin.','error');return}const type=f.elements.type.value,dayPortion=f.elements.dayPortion.value;if(dayPortion!=='full'&&(type!=='vacation'||f.elements.from.value!==f.elements.to.value)){toast('Ein halber Tag ist nur bei einem eintägigen Urlaub möglich.','error');return}const days=requestedDays(f.elements.from.value,f.elements.to.value,user.workDays,dayPortion,type==='vacation'?'Urlaub':type);await addDoc(collection(db,'absences'),{userId:user.id,userName:user.name||user.email||user.id,companyId:user.companyId||'',supervisorId:user.supervisorId||null,supervisorId2:user.supervisorId2||null,type,from:f.elements.from.value,to:f.elements.to.value,days,dayPortion,certificateStatus:type==='vacation'?'':f.elements.certificateStatus.value,note:f.elements.note.value.trim(),status:type==='vacation'?'active':null,source:'hr_direct',createdBy:ctx.profile.id,createdByName:ctx.profile.name||ctx.profile.email||'',createdAt:serverTimestamp()});toast(type==='vacation'?'Urlaub sofort gültig gebucht.':'Abwesenheit gebucht.');renderUrlaub(el,ctx)};
  el.querySelectorAll('.withdraw-direct-vacation').forEach(b=>b.onclick=async()=>{if(!await confirmDialog('Diesen direkt gebuchten Urlaub wirklich zurückziehen? Der Vorgang bleibt zur Nachvollziehbarkeit protokolliert.'))return;await updateDoc(doc(db,'absences',b.dataset.id),{status:'withdrawn',withdrawnAt:serverTimestamp(),withdrawnBy:ctx.profile.id,withdrawnByName:ctx.profile.name||ctx.profile.email||''});toast('Direkt gebuchter Urlaub zurückgezogen.');renderUrlaub(el,ctx)});
  el.querySelectorAll('.delete-absence').forEach(b=>b.onclick=async()=>{if(!await confirmDialog('Diese Abwesenheitsbuchung wirklich löschen?'))return;await deleteDoc(doc(db,'absences',b.dataset.id));toast('Abwesenheit gelöscht.');renderUrlaub(el,ctx)});
}
