...
el.querySelectorAll('.reset-password').forEach(b=>b.onclick=async()=>{
    ...
    const password=await inputDialog(`Neues Startpasswort für ${u.name||u.username||'Mitarbeiter'} vergeben:`,'',{title:'Passwort zurücksetzen',inputType:'text',placeholder:'mindestens 6 Zeichen'});
    if(password===null)return;
    const value=String(password||'');
    if(value.length<6){toast('Das neue Startpasswort muss mindestens 6 Zeichen lang sein.','error');return;}
  ...
  form.onsubmit=async e=>{
    ...
    if(!id){
      ...
      if(password.length<6)issues.push('• Startpasswort: mindestens 6 Zeichen eingeben.');
    }
