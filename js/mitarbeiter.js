...
export async function renderMitarbeiter(el,ctx){
  ...
  const employmentBody=`
    ...
    <div class="field full"><span>Position / Tätigkeit · Karriereverlauf</span><div class="history-entry-grid"><input name="position" placeholder="1. Position / Tätigkeit"><input name="positionDate1" type="date" aria-label="Gültig ab Position 1"><input name="position2" placeholder="2. Position / Tätigkeit"><input name="positionDate2" type="date" aria-label="Gültig ab Position 2"><input name="position3" placeholder="3. Position / Tätigkeit"><input name="positionDate3" type="date" aria-label="Gültig ab Position 3"><input name="position4" placeholder="4. Position / Tätigkeit"><input name="positionDate4" type="date" aria-label="Gültig ab Position 4"></div><small>Der Karriereverlauf bleibt erhalten. Ab dem jeweiligen Datum gilt automatisch die neue Position.</small></div><label class="field"><span>Kostenstelle</span><input name="costCenter"></label>
  ...
  const timeBody=`
    <label class="field"><span>Wochenstunden</span><input name="weeklyHours" type="number" step="0.25" value="40"></label><div class="field full"><span>Urlaubsanspruch / Jahr</span><div class="history-entry-grid vacation-entitlement-grid"><input name="vacationDays" type="number" step="0.5" min="0" value="30" aria-label="Grundanspruch Urlaubstage"><span class="history-fixed-date">ab Beschäftigungsbeginn</span><input name="vacationDays2" type="number" step="0.5" min="0" placeholder="2. Anspruch" aria-label="Urlaubsanspruch Stufe 2"><input name="vacationDate2" type="date" aria-label="Gültig ab Urlaubsanspruch Stufe 2"><input name="vacationDays3" type="number" step="0.5" min="0" placeholder="3. Anspruch" aria-label="Urlaubsanspruch Stufe 3"><input name="vacationDate3" type="date" aria-label="Gültig ab Urlaubsanspruch Stufe 3"><input name="vacationDays4" type="number" step="0.5" min="0" placeholder="4. Anspruch" aria-label="Urlaubsanspruch Stufe 4"><input name="vacationDate4" type="date" aria-label="Gültig ab Urlaubsanspruch Stufe 4"></div><small>Der erste Anspruch gilt ab Beschäftigungsbeginn. Weitere Stufen werden am eingetragenen Datum automatisch wirksam und ersetzen ab dann den vorherigen Anspruch.</small></div><label class="field"><span>Frühester anrechenbarer Arbeitsbeginn</span><input name="earliestStartTime" type="time"><small>Optional. Ein früherer echter KOMMEN-Stempel bleibt sichtbar, wird aber erst ab dieser Uhrzeit angerechnet.</small></label><label class="field"><span>Zeiterfassung auf Projekte</span><select name="projectTimeTracking"><option value="false">Nein</option><option value="true">Ja</option></select></label>
...
el.querySelectorAll('.reset-password').forEach(b=>b.onclick=async()=>{
  ...
  form.onsubmit=async e=>{
    ...
    const vacationEntitlements=[];for(let i=1;i<=4;i++){const daysRaw=f.elements[i===1?'vacationDays':`vacationDays${i}`]?.value,validFrom=i===1?'':(f.elements[`vacationDate${i}`]?.value||'');if(daysRaw!==''&&daysRaw!=null){const days=Number(daysRaw);if(!Number.isFinite(days)||days<0)issues.push(`• Urlaubsanspruch Stufe ${i}: Bitte eine gültige Zahl eingeben.`);else if(i>1&&!validFrom)issues.push(`• Urlaubsanspruch Stufe ${i}: Bitte „gültig ab“ angeben.`);else vacationEntitlements.push({days,validFrom});}else if(i>1&&validFrom)issues.push(`• Urlaubsanspruch Stufe ${i}: Bitte Urlaubstage ergänzen.`);}for(let i=1;i<vacationEntitlements.length;i++){if(vacationEntitlements[i].validFrom<=vacationEntitlements[i-1].validFrom)issues.push('• Die Datumswerte der Urlaubsansprüche müssen chronologisch aufsteigend sein.');}
    const positionHistory=[];for(let i=1;i<=4;i++){const position=(f.elements[i===1?'position':`position${i}`]?.value||'').trim(),validFrom=f.elements[`positionDate${i}`]?.value||'';if(position){if(!validFrom)issues.push(`• Position ${i}: Bitte „gültig ab“ angeben.`);else positionHistory.push({position,validFrom});}else if(validFrom)issues.push(`• Position ${i}: Bitte die Tätigkeit/Position ergänzen.`);}for(let i=1;i<positionHistory.length;i++){if(positionHistory[i].validFrom<=positionHistory[i-1].validFrom)issues.push('• Die Datumswerte des Karriereverlaufs müssen chronologisch aufsteigend sein.');}
