...
export async function renderMitarbeiter(el,ctx){
  ...
  function openEmployeeMasterDataSheet(employee){
    ...
    const sections=[
      ...
      printSection('System & Zugang',[
        ['Login',login],['Rolle',ROLE_LABELS[employee.role]||employee.role],['TP-Managementportal',employee.managementPortalAccess===true?'berechtigt':'nicht berechtigt'],['Kontostatus',employee.active===false?'inaktiv':'aktiv'],['Admin-Berechtigungen',permissions,true]
      ])
    ].join('');
  ...
  const systemBody=`
    <label class="field"><span>Login-Art</span><select name="loginType"><option value="email">E-Mail-Adresse</option><option value="username">Benutzername</option></select></label><label class="field"><span>E-Mail / Benutzername</span><input name="login" required></label><label class="field"><span>Startpasswort</span><input name="password" type="text" placeholder="nur bei Neuanlage"></label><label class="field"><span>Rolle</span><select name="role"><option value="employee">Mitarbeiter</option><option value="supervisor">Vorgesetzter</option><option value="admin">Personalabteilung / Admin</option></select></label><label class="field"><span>Status</span><select name="active"><option value="true">aktiv</option><option value="false">inaktiv</option></select></label><label class="field full permission-toggle"><span>Systemberechtigungen</span><label class="permission-option"><input type="checkbox" name="managementPortalAccess"><span><strong>Berechtigt für TP-Managementportal</strong><small>Der Mitarbeiter darf sich mit seinem bestehenden TP-Personalmanagement-Zugang auch am TP-Managementportal anmelden.</small></span></label></label>${adminPermissionBody}${supervisorPermissionBody}`;
