import { firebaseConfig, db, functions } from "./firebase.js";
...
async function createAuthAccount(email,password){
  ...
  try{
    ...
    try{await deleteApp(app)}catch(cleanupErr){console.warn('Zweites Firebase-App-Objekt konnte nicht geschlossen werden',cleanupErr)}
    if(err?.code==='auth/email-already-in-use')throw new Error('Für diesen Login existiert bereits ein Authentication-Konto. Bitte den vorhandenen Zugang prüfen; ein weiterer Mitarbeiter wird nicht angelegt.');
    if(err?.code==='auth/invalid-email')throw new Error('Der Login kann nicht als gültige E-Mail-Adresse für Authentication verwendet werden. Bitte E-Mail-Adresse bzw. Benutzernamen prüfen.');
    throw err;
  }
}
...
function boolVal(form,name,value){const input=form?.elements?.[name];if(!input)return;if(input.type==='checkbox')input.checked=value===true;else input.value=String(value===true)}
...
export async function renderMitarbeiter(el,ctx){
  ...
  function printMoney(value,suffix=''){
    if(value===null||value===undefined||value==='')return '–';
    const n=Number(value);
    return Number.isFinite(n)?`${new Intl.NumberFormat('de-DE',{style:'currency',currency:'EUR'}).format(n)}${suffix}`:'–';
  }
  function printSection(title,rows){
    return `<section class="sheet-section"><h2>${esc(title)}</h2><div class="sheet-grid">${rows.map(([label,value,wide=false])=>`<div class="sheet-row${wide?' wide':''}"><span>${esc(label)}</span><strong>${esc(printValue(value))}</strong></div>`).join('')}</div></section>`;
  }
  function openEmployeeMasterDataSheet(employee){
    if(!employee?.id)return;
    const p=privateMap.get(employee.id)||{};
    const company=companies.find(c=>c.id===employee.companyId);
    const area=businessAreaMap.get(employee.businessAreaId);
    const supervisor1=activeUsers.find(u=>u.id===employee.supervisorId);
    const supervisor2=activeUsers.find(u=>u.id===employee.supervisorId2);
    const insurer=insurers.find(x=>x.id===p.healthInsuranceId);
    const bank=banks.find(x=>x.id===p.bankId);
    const religion=religions.find(x=>x.code===p.religion);
    const workDayNames={'1':'Montag','2':'Dienstag','3':'Mittwoch','4':'Donnerstag','5':'Freitag','6':'Samstag','0':'Sonntag'};
    const selectedWorkDays=(employee.workDays||[]).map(String).map(x=>workDayNames[x]||x).join(', ')||'–';
    const selectedAreas=(employee.bereiche||[]).map(id=>AREA_NAMES[id]?`${id} · ${AREA_NAMES[id]}`:id).join(', ')||'–';
    const selectedExtra=(employee.extraTrainings||[]).map(id=>trainings.find(t=>t.id===id)?.title||id).join(', ')||'–';
    const permissions=employee.role==='admin'?ADMIN_PERMISSION_DEFS.filter(def=>normalizedAdminPermissions(employee)[def.key]!==false).map(def=>def.label||def.name||def.key).join(', '):'–';
    const login=employee.hasRealEmail===false?(employee.username||'–'):(employee.email||'–');
    const logoUrl=new URL('./assets/tp-logo.png',window.location.href).href;
    const now=new Date().toLocaleString('de-DE',{dateStyle:'medium',timeStyle:'short'});
    const sections=[
      printSection('Persönliche Daten & Kontakt',[
        ['Name (Anzeige)',employee.name],['Anrede',p.salutation],['Titel',p.title],['Vorname',p.firstName],['Nachname',p.lastName],['Geburtsdatum',printDate(p.birthDate)],['Geschlecht',p.gender],['Geburtsname',p.birthName],['Geburtsort',p.birthPlace],['Geburtsnationalität',p.birthNationality],['Geburtstagsliste',p.birthdayList===true?'Ja':'Nein'],['Straße / Hausnummer',p.street,true],['PLZ',p.postalCode],['Ort',p.city],['Private E-Mail',p.privateEmail],['Telefon',p.phone],['Mobil',p.mobile],['Notfallkontakt',p.emergencyContactName],['Telefon Notfallkontakt',p.emergencyContactPhone]
      ]),
      printSection('Beschäftigung & Organisation',[
        ['Firma',company?.name||employee.companyId],['Mitarbeiternummer',employee.employeeNumber],['Geschäftsbereich',area?`${area.code} · ${area.name}`:(employee.companyAreaNumber||'')],['Abteilung',employee.department],['Aktuelle Position / Tätigkeit',positionOn(employee)],['Kostenstelle',employee.costCenter],['1. Vorgesetzter',supervisor1?.name||'–'],['2. Vorgesetzter',supervisor2?.name||'–'],['Beschäftigungsart',employee.contractType],['Eintritt',printDate(employee.startDate)],['Austritt',printDate(employee.endDate)],['Probezeit bis',printDate(employee.probationEndDate)],['Befristung bis',printDate(employee.fixedTermEndDate)]
      ]),
      printSection('Steuer & Sozialversicherung',[
        ['Familienstand',p.maritalStatus],['Heirat am',printDate(p.marriageDate)],['Steuer-ID',p.taxId],['Steuerklasse',p.taxClass],['Kinderfreibetrag',p.childAllowance],['Religion / Kirchensteuer',religion?`${religion.code} · ${religion.name}`:p.religion],['Krankenversicherungsnummer',p.healthInsuranceNumber],['Sozialversicherungsnummer',p.socialSecurityNumber],['Krankenkasse',insurer?`${insurer.name}${insurer.code?` · ${insurer.code}`:''}`:p.healthInsuranceId],['Versicherungsart',p.insuranceType],['Personengruppe',p.personGroup],['Beitragsgruppe',p.contributionGroup]
      ]),
      printSection('Bankverbindung',[
        ['IBAN',p.iban,true],['Bank',bank?.name||p.bankId],['BIC',p.bic],['Kontoinhaber',p.accountHolder,true]
      ]),
      printSection('Lohn & Gehalt',[
        ['Bruttogehalt / Monat',printMoney(p.grossSalary)],['Stundenlohn',printMoney(p.hourlyRate,' / h')],['Gültig ab',printDate(p.salaryValidFrom)]
      ]),
      printSection('Urlaub & Arbeitszeit',[
        ['Wochenstunden',employee.weeklyHours],['Aktueller Urlaubsanspruch / Jahr',vacationEntitlementOn(employee)],['Frühester anrechenbarer Arbeitsbeginn',employee.earliestStartTime?`${employee.earliestStartTime} Uhr`:'–'],['Zeiterfassung auf Projekte',employee.projectTimeTracking===true?'Ja':'Nein'],['Pauschale 8h-Darstellung für Mitarbeiter',employee.flatEightHourEmployeeView===true?'Ja':'Nein'],['Mitarbeiter ohne Zeiterfassung',employee.noTimeTracking===true?'Ja':'Nein'],['Regelmäßige Arbeitstage',selectedWorkDays,true]
      ]),
      printSection('Arbeitssicherheit & Befähigungen',[
        ['Ersthelfer',employee.firstAider===true?'Ja':'Nein'],['Ersthelfer gültig / Auffrischung bis',printDate(employee.firstAiderValidUntil)],['Brandschutzhelfer',employee.fireWarden===true?'Ja':'Nein'],['Brandschutzhelfer gültig / Auffrischung bis',printDate(employee.fireWardenValidUntil)],['Staplerschein',employee.forkliftPermit===true?'Ja':'Nein'],['Staplerschein gültig bis',printDate(employee.forkliftPermitValidUntil)],['Hubarbeitsbühne',employee.aerialLiftPermit===true?'Ja':'Nein'],['Hubarbeitsbühne gültig bis',printDate(employee.aerialLiftPermitValidUntil)],['Führerscheinklassen',employee.drivingLicenseClasses],['Nächste Führerscheinkontrolle',printDate(employee.nextDrivingLicenseCheck)],['Arbeitsmedizinische Vorsorgen / Hinweise',employee.occupationalMedicalNotes,true]
      ]),
      printSection('Schulungen',[
        ['Schulungsbereiche',selectedAreas,true],['Individuelle Zusatzschulungen',selectedExtra,true]
      ]),
      printSection('System & Zugang',[
        ['Login',login],['Rolle',ROLE_LABELS[employee.role]||employee.role],['TP-Managementportal',employee.managementPortalAccess===true?'berechtigt':'nicht berechtigt'],['Dokumentenrechte Managementportal',employee.managementPortalDocumentAccess===true?'berechtigt':'nur Lesen'],['Kontostatus',employee.active===false?'inaktiv':'aktiv'],['Admin-Berechtigungen',permissions,true]
      ])
    ].join('');
    const w=window.open('','_blank');
    if(!w){toast('Das Stammdatenblatt konnte nicht geöffnet werden. Bitte Pop-ups für diese Seite erlauben.');return;}
    w.document.open();
    w.document.write(`<!doctype html><html lang="de"><head><meta charset="utf-8"><title>Mitarbeiter-Stammdatenblatt · ${esc(employee.name||'Mitarbeiter')}</title><style>
      *{box-sizing:border-box}body{margin:0;font-family:Arial,Helvetica,sans-serif;color:#172033;background:#eef2f6}main{width:210mm;min-height:297mm;margin:18px auto;background:#fff;padding:14mm 15mm 16mm;box-shadow:0 4px 24px rgba(0,0,0,.12)}
      .toolbar{position:sticky;top:0;display:flex;gap:8px;justify-content:flex-end;padding:10px 16px;background:#172033}.toolbar button{border:0;border-radius:7px;padding:9px 14px;font-weight:700;cursor:pointer}.toolbar .primary{background:#fff;color:#17365d}.toolbar .secondary{background:#dfe7ef;color:#172033}
      header{display:flex;justify-content:space-between;gap:20px;align-items:flex-start;border-bottom:3px solid #17365d;padding-bottom:10px;margin-bottom:14px}header img{width:92px;max-height:50px;object-fit:contain}h1{font-size:22px;margin:0 0 5px;color:#17365d}.sub{font-size:11px;color:#667085;line-height:1.45}.identity{text-align:right;font-size:12px}.identity strong{display:block;font-size:15px;color:#172033;margin-bottom:3px}
      .confidential{margin:0 0 12px;padding:7px 9px;border:1px solid #d5dde6;background:#f7f9fb;border-radius:5px;font-size:10px;color:#526173}.sheet-section{break-inside:avoid;margin:0 0 12px}.sheet-section h2{font-size:13px;color:#17365d;margin:0;padding:6px 8px;background:#edf3f8;border-left:4px solid #17365d}.sheet-grid{display:grid;grid-template-columns:1fr 1fr;border-left:1px solid #d8e0e8}.sheet-row{display:grid;grid-template-columns:44% 56%;min-height:27px;border-right:1px solid #d8e0e8;border-bottom:1px solid #d8e0e8}.sheet-row.wide{grid-column:1/-1}.sheet-row span,.sheet-row strong{padding:5px 7px;font-size:10px;line-height:1.35;white-space:pre-wrap;overflow-wrap:anywhere}.sheet-row span{background:#f7f9fb;color:#5d6a79;font-weight:700}.sheet-row strong{font-weight:500}.footer-note{font-size:9px;color:#788596;margin-top:14px;border-top:1px solid #d8e0e8;padding-top:7px}
      @page{size:A4;margin:10mm}@media print{body{background:#fff}.toolbar{display:none}main{width:auto;min-height:0;margin:0;padding:0;box-shadow:none}.sheet-section{break-inside:avoid-page}}
    </style></head><body><div class="toolbar"><button class="primary" onclick="window.print()">Drucken / als PDF speichern</button><button class="secondary" onclick="window.close()">Schließen</button></div><main><header><div><img src="${esc(logoUrl)}" alt="TP"><h1>Mitarbeiter-Stammdatenblatt</h1><div class="sub">TP-Personalmanagement · Version 2.13.1</div></div><div class="identity"><strong>${esc(employee.name||'Mitarbeiter')}</strong>Mitarbeiternummer: ${esc(employee.employeeNumber||'–')}<br>Stand: ${esc(now)}</div></header><p class="confidential"><strong>Vertrauliche Personaldaten.</strong> Dieses Stammdatenblatt enthält die im TP-Personalmanagement gespeicherten Stammdaten des ausgewählten Mitarbeiters.</p>${sections}<p class="footer-note">Erstellt über TP-Personalmanagement · Die Ausgabe kann über den Browser gedruckt oder im Druckdialog als PDF gespeichert werden.</p></main></body></html>`);
    w.document.close();
  }

  const personalBody=`
    <label class="field"><span>Name (Anzeige) <b class="required-mark" aria-label="Pflichtfeld">*</b></span><input name="name" required></label><div class="field birthday-date-field"><span>Geburtsdatum</span><div class="birthday-date-line"><input name="birthDate" type="date"><label class="inline-check compact"><input name="birthdayList" type="checkbox"><span>Geburtstagsliste ja</span></label></div><small>Nur bei Zustimmung wird der Geburtstag in der Geburtstagsliste ausgegeben.</small></div>
    <label class="field"><span>Anrede</span><select name="salutation"><option value="">– auswählen –</option><option>Herr</option><option>Frau</option><option>Divers</option><option>Keine Angabe</option></select></label><label class="field"><span>Titel</span><input name="title"></label>
    <label class="field"><span>Vorname</span><input name="firstName"></label><label class="field"><span>Nachname</span><input name="lastName"></label>
    <label class="field"><span>Geschlecht</span><select name="gender"><option value="">– auswählen –</option><option value="männlich">männlich</option><option value="weiblich">weiblich</option><option value="divers">divers</option><option value="keine Angabe">keine Angabe</option></select></label><label class="field"><span>Geburtsname</span><input name="birthName"></label>
    <label class="field"><span>Geburtsort</span><input name="birthPlace"></label><label class="field"><span>Geburtsnationalität</span><input name="birthNationality"></label>
  ...
  const employmentBody=`
    <label class="field"><span>Firma <b class="required-mark" aria-label="Pflichtfeld">*</b></span><select name="companyId" required><option value="">– auswählen –</option>${companies.map(c=>opt(c.id,`${c.name}${c.companyNumber?` · ${c.companyNumber}`:''}`)).join('')}</select></label><label class="field"><span>Mitarbeiternummer <b class="required-mark" aria-label="Pflichtfeld">*</b></span><input name="employeeNumber" inputmode="numeric" maxlength="5" pattern="[0-9]{5}" placeholder="5-stellig, z. B. 40190" required></label>
    <label class="field"><span>Geschäftsbereich <b class="required-mark" aria-label="Pflichtfeld">*</b></span><select name="businessAreaId" required><option value="">– auswählen –</option>${businessAreas.map(a=>opt(a.id,`${a.code} · ${a.name}`)).join('')}</select></label><label class="field"><span>Abteilung</span><input name="department"></label>
    <div class="field full"><span>Position / Tätigkeit · Karriereverlauf <b class="required-mark" aria-label="Pflichtfeld">*</b></span><div id="position-history-rows" class="dynamic-history-list"></div><button class="btn secondary small history-add-btn" id="add-position-history" type="button">+ Weitere Position hinzufügen</button><small>Der Karriereverlauf bleibt erhalten. Ab dem jeweiligen Datum gilt automatisch die neue Position.</small></div><label class="field"><span>Kostenstelle</span><input name="costCenter"></label>
  ...
  const taxBody=`
    ...
    <label class="field"><span>Krankenversicherungsnummer</span><input name="healthInsuranceNumber" autocomplete="off"></label><label class="field"><span>Sozialversicherungsnummer</span><input name="socialSecurityNumber" autocomplete="off"></label>
    <label class="field"><span>Krankenkasse</span><select name="healthInsuranceId"><option value="">– auswählen –</option>${insurers.map(x=>opt(x.id,`${x.name}${x.code?` · ${x.code}`:''}`)).join('')}</select></label>
  ...
  const timeBody=`
    <input name="weeklyHours" type="hidden" value="40"><div class="field full"><span>Arbeitszeitregelungen · zeitlicher Verlauf</span><div id="work-schedule-rows" class="work-schedule-history"></div><button class="btn secondary small history-add-btn" id="add-work-schedule" type="button">+ Weitere Arbeitszeitregelung hinzufügen</button><small>Jede Regelung gilt ab dem angegebenen Datum. Frühere Regeln bleiben erhalten; zukünftige Regeln können vorbereitet werden. Rückwirkende Korrekturen verändern die Neuberechnung des Stundenkontos und werden protokolliert.</small></div><div class="field full"><span>Urlaubsanspruch / Jahr</span><div id="vacation-entitlement-rows" class="dynamic-history-list"></div><button class="btn secondary small history-add-btn" id="add-vacation-entitlement" type="button">+ Weiteren Urlaubsanspruch hinzufügen</button><small>Der erste Anspruch gilt ab Beschäftigungsbeginn. Weitere Stufen werden am eingetragenen Datum automatisch wirksam und ersetzen ab dann den vorherigen Anspruch.</small></div><label class="field"><span>Frühester anrechenbarer Arbeitsbeginn</span><input name="earliestStartTime" type="time"><small>Optional. Ein früherer echter KOMMEN-Stempel bleibt sichtbar, wird aber erst ab dieser Uhrzeit angerechnet.</small></label><label class="field"><span>Zeiterfassung auf Projekte</span><select name="projectTimeTracking"><option value="false">Nein</option><option value="true">Ja</option></select></label>
    <div class="field full"><span>Besondere Pausenregelung</span><label class="inline-check compact"><input name="specialPauseActive" type="checkbox"><span>Feste Pause verwenden</span></label><div class="form-grid"><label class="field"><span>Pausenminuten</span><input name="specialPauseMinutes" type="number" min="30" step="1" value="30"></label><label class="field"><span>Gültig ab</span><input name="specialPauseValidFrom" type="date"></label></div><small>Die feste Pause ersetzt ab dem Datum die automatische 30-/45-Minuten-Regel. Zum Beenden Checkbox deaktivieren und ein neues Gültigkeitsdatum speichern.</small></div>
  ...
  const supervisorPermissionBody=`<div class="field full admin-permissions-box hidden" id="supervisor-permissions-box"><span>Vorgesetzten-Berechtigungen</span><p class="muted small">Diese Berechtigungen gelten für alle dem Vorgesetzten als 1. oder 2. Vorgesetzten zugeordneten Mitarbeiter. Bestehende Vorgesetzte behalten nach dem Update zunächst den bisherigen Zugriff.</p><div class="permission-grid">${SUPERVISOR_PERMISSION_DEFS.map(x=>`<label><input type="checkbox" name="supervisorPermission" value="${x.key}" checked><span>${esc(x.label)}</span></label>`).join('')}</div></div>`;
  const systemBody=`
    <label class="field"><span>Login-Art</span><select name="loginType"><option value="email">E-Mail-Adresse</option><option value="username">Benutzername</option></select></label><label class="field"><span><span id="employee-login-label">E-Mail-Adresse</span> <b class="required-mark" aria-label="Pflichtfeld">*</b></span><input name="login" required></label><label class="field"><span>Startpasswort <b class="required-mark new-user-required" aria-label="Pflichtfeld">*</b></span><input name="password" type="text" minlength="6" placeholder="mind. 8 Zeichen inkl. Groß/Klein, Zahl, Sonderzeichen"><small>Bei Neuanlage erforderlich: mindestens 8 Zeichen, Groß- und Kleinbuchstaben, mindestens 1 Zahl und 1 Sonderzeichen. Beim ersten Login muss ein persönliches Passwort festgelegt werden.</small></label><label class="field"><span>Rolle</span><select name="role"><option value="employee">Mitarbeiter</option><option value="supervisor">Vorgesetzter</option><option value="admin">Personalabteilung / Admin</option></select></label><label class="field"><span>Status</span><select name="active"><option value="true">aktiv</option><option value="false">inaktiv</option></select></label><div class="field full permission-toggle"><span>Systemberechtigungen</span><div class="permission-options"><label class="permission-option"><input type="checkbox" name="managementPortalAccess"><span><strong>Berechtigt für TP-Managementportal</strong><small>Der Mitarbeiter darf sich mit seinem bestehenden TP-Personalmanagement-Zugang auch am TP-Managementportal anmelden.</small></span></label><label class="permission-option" id="management-portal-document-option"><input type="checkbox" name="managementPortalDocumentAccess"><span><strong>Dokumentenrechte im TP-Managementportal</strong><small>Für Büromitarbeiter: Dokumente erstellen und hochladen sowie zugewiesene Prüf- und Freigabeaufgaben bearbeiten. Ohne diese Freigabe besteht nur Lesezugriff. Vorgesetzte und Admins erhalten diese Rechte im Managementportal automatisch.</small></span></label></div></div>${adminPermissionBody}${supervisorPermissionBody}`;
  ...
  <dialog id="pds-import-dialog" class="pds-import-dialog"><form method="dialog"><div class="pds-import-head"><div><span class="eyebrow">PDS-Lohn</span><h3>Mitarbeiterdaten aus PDF prüfen</h3><p>Die Werte wurden nur aus der OCR-Textebene gelesen und noch nicht gespeichert.</p></div><button class="btn secondary small" value="cancel" type="submit">Schließen</button></div><div id="pds-import-warnings"></div><div id="pds-import-preview"></div><div class="actions pds-import-actions"><button class="btn primary" id="pds-import-apply" type="button">Daten übernehmen</button><button class="btn secondary" value="cancel" type="submit">Abbrechen</button></div></form></dialog>`;

  const show=el.querySelector('#user-show'),create=el.querySelector('#user-create'),form=el.querySelector('#user-form'),akte=el.querySelector('#personalakte-container'),pdsImportBtn=el.querySelector('#pds-pdf-import'),masterDataPrintBtn=el.querySelector('#employee-masterdata-print'),pdsFile=el.querySelector('#pds-pdf-file'),pdsDialog=el.querySelector('#pds-import-dialog');
  let formMode='new',editingUserId=null;
  const positionHistoryRows=form.querySelector('#position-history-rows'),vacationEntitlementRows=form.querySelector('#vacation-entitlement-rows'),workScheduleRows=form.querySelector('#work-schedule-rows');
  const WORK_DAYS=[['1','Montag'],['2','Dienstag'],['3','Mittwoch'],['4','Donnerstag'],['5','Freitag'],['6','Samstag'],['0','Sonntag']];
  function scheduleMinutesText(minutes){const n=Math.max(0,Math.round(Number(minutes)||0));return `${Math.floor(n/60)}:${String(n%60).padStart(2,'0')} h`}
  function workScheduleRow(row={},index=0){const wrap=document.createElement('article');wrap.className='work-schedule-row';wrap.dataset.historyRow='workSchedule';const model=row.model==='individual'?'individual':'uniform',daily=row.dailyMinutes||{},days=new Set((row.workDays?.length?row.workDays:['1','2','3','4','5']).map(String));wrap.innerHTML=`<div class="work-schedule-head"><strong>Regelung ${index+1}</strong>${index?'<button class="btn secondary small history-remove-btn" type="button">Entfernen</button>':''}</div><div class="work-schedule-meta"><label class="field"><span>Gültig ab</span><input data-schedule="validFrom" type="date" value="${esc(row.validFrom||'')}"></label><label class="field"><span>Arbeitszeitmodell</span><select data-schedule="model"><option value="uniform" ${model==='uniform'?'selected':''}>Gleichmäßige Verteilung</option><option value="individual" ${model==='individual'?'selected':''}>Individuelle Sollstunden je Wochentag</option></select></label><label class="field"><span>Anlass</span><select data-schedule="changeType"><option value="change" ${row.changeType!=='correction'?'selected':''}>Vertragliche Änderung</option><option value="correction" ${row.changeType==='correction'?'selected':''}>Korrektur einer fehlerhaften Regelung</option></select></label></div><div class="uniform-schedule-fields"><label class="field"><span>Vertragliche Wochenstunden</span><input data-schedule="weeklyHours" type="number" min="0" step="0.25" value="${esc(row.weeklyHours??40)}"></label><div class="field full"><span>Regelmäßige Arbeitstage</span><div class="weekday-grid">${WORK_DAYS.map(([value,label])=>`<label><input data-schedule-day="${value}" type="checkbox" ${days.has(value)?'checked':''}><span>${label.slice(0,2)}</span></label>`).join('')}</div></div></div><div class="individual-schedule-fields"><div class="individual-day-grid">${WORK_DAYS.map(([value,label])=>{const minutes=Math.max(0,Math.round(Number(daily[value])||0)),time=`${String(Math.floor(minutes/60)).padStart(2,'0')}:${String(minutes%60).padStart(2,'0')}`;return `<label class="individual-day"><input data-individual-day="${value}" type="checkbox" ${minutes>0?'checked':''}><span>${label}</span><input data-individual-time="${value}" type="time" step="300" value="${minutes>0?time:''}" ${minutes>0?'':'disabled'}></label>`}).join('')}</div><div class="work-schedule-total">Wochenstunden: <strong data-schedule-total>0:00 h</strong></div></div><label class="field full"><span>Hinweis zur Regelung (optional)</span><input data-schedule="note" value="${esc(row.note||'')}" placeholder="z. B. Wechsel von Werkstudent zu Vollzeit"></label>`;const sync=()=>{const individual=wrap.querySelector('[data-schedule="model"]').value==='individual';wrap.classList.toggle('is-individual',individual);wrap.querySelectorAll('[data-individual-day]').forEach(box=>{const input=wrap.querySelector(`[data-individual-time="${box.dataset.individualDay}"]`);input.disabled=!box.checked;if(!box.checked)input.value=''});let total=0;if(individual)wrap.querySelectorAll('[data-individual-time]').forEach(input=>{if(!input.disabled&&input.value){const [h,m]=input.value.split(':').map(Number);total+=(h||0)*60+(m||0)}});else total=Math.round((Number(wrap.querySelector('[data-schedule="weeklyHours"]').value)||0)*60);wrap.querySelector('[data-schedule-total]').textContent=scheduleMinutesText(total)};wrap.addEventListener('input',sync);wrap.addEventListener('change',sync);wrap.querySelector('.history-remove-btn')?.addEventListener('click',()=>{wrap.remove();[...workScheduleRows.children].forEach((x,i)=>{const title=x.querySelector('.work-schedule-head strong');if(title)title.textContent=`Regelung ${i+1}`})});sync();return wrap}
  function renderWorkSchedules(rows=[]){workScheduleRows.innerHTML='';const list=rows.length?rows:[{model:'uniform',validFrom:new Date().toISOString().slice(0,10),weeklyHours:40,workDays:['1','2','3','4','5']}];list.forEach((row,i)=>workScheduleRows.appendChild(workScheduleRow(row,i)))}
  function collectWorkSchedules(issues){const out=[...workScheduleRows.querySelectorAll('[data-history-row="workSchedule"]')].map((row,index)=>{const model=row.querySelector('[data-schedule="model"]').value,validFrom=row.querySelector('[data-schedule="validFrom"]').value,changeType=row.querySelector('[data-schedule="changeType"]').value,note=row.querySelector('[data-schedule="note"]').value.trim();let weeklyHours=0,workDays=[],dailyMinutes={};if(model==='individual'){row.querySelectorAll('[data-individual-day]').forEach(box=>{const day=box.dataset.individualDay,input=row.querySelector(`[data-individual-time="${day}"]`);let minutes=0;if(box.checked&&input.value){const [h,m]=input.value.split(':').map(Number);minutes=(h||0)*60+(m||0)}dailyMinutes[day]=minutes;if(minutes>0)workDays.push(day)});weeklyHours=Object.values(dailyMinutes).reduce((sum,n)=>sum+n,0)/60}else{weeklyHours=Number(row.querySelector('[data-schedule="weeklyHours"]').value);workDays=[...row.querySelectorAll('[data-schedule-day]:checked')].map(x=>x.dataset.scheduleDay)}return {model,validFrom,weeklyHours,workDays,dailyMinutes,changeType,note,index:index+1}});out.forEach(row=>{if(!row.validFrom)issues.push(`• Arbeitszeitregelung ${row.index}: Bitte „Gültig ab“ angeben.`);if(!Number.isFinite(row.weeklyHours)||row.weeklyHours<=0)issues.push(`• Arbeitszeitregelung ${row.index}: Die Wochenstunden müssen größer als 0 sein.`);if(!row.workDays.length)issues.push(`• Arbeitszeitregelung ${row.index}: Bitte mindestens einen Arbeitstag mit Sollzeit auswählen.`)});for(let i=1;i<out.length;i++)if(out[i].validFrom<=out[i-1].validFrom)issues.push('• Die Arbeitszeitregelungen müssen chronologisch aufsteigend sein und unterschiedliche Datumswerte besitzen.');return out.map(({index,...row})=>row)}
  function positionRow(row={},index=0){const wrap=document.createElement('div');wrap.className='dynamic-history-row';wrap.dataset.historyRow='position';wrap.innerHTML=`<input data-history-value="position" placeholder="Position / Tätigkeit" aria-label="Position / Tätigkeit" value="${esc(row.position||'')}"><input data-history-date="position" type="date" aria-label="Gültig ab" value="${esc(row.validFrom||'')}">${index?'<button class="btn secondary small history-remove-btn" type="button" aria-label="Position entfernen">Entfernen</button>':'<span class="history-row-spacer"></span>'}`;wrap.querySelector('.history-remove-btn')?.addEventListener('click',()=>wrap.remove());return wrap}
  function vacationRow(row={},index=0){const wrap=document.createElement('div');wrap.className='dynamic-history-row';wrap.dataset.historyRow='vacation';if(index===0)wrap.innerHTML=`<input name="vacationDays" data-history-value="vacation" type="number" step="0.5" min="0" value="${esc(row.days??30)}" aria-label="Grundanspruch Urlaubstage"><span class="history-fixed-date">ab Beschäftigungsbeginn</span><span class="history-row-spacer"></span>`;else wrap.innerHTML=`<input data-history-value="vacation" type="number" step="0.5" min="0" placeholder="Urlaubstage" value="${esc(row.days??'')}" aria-label="Urlaubsanspruch"><input data-history-date="vacation" type="date" aria-label="Gültig ab" value="${esc(row.validFrom||'')}"><button class="btn secondary small history-remove-btn" type="button" aria-label="Urlaubsanspruch entfernen">Entfernen</button>`;wrap.querySelector('.history-remove-btn')?.addEventListener('click',()=>wrap.remove());return wrap}
  function renderPositionHistory(rows=[]){positionHistoryRows.innerHTML='';const list=rows.length?rows:[{position:'',validFrom:''}];list.forEach((row,i)=>positionHistoryRows.appendChild(positionRow(row,i)))}
  function renderVacationEntitlements(rows=[]){vacationEntitlementRows.innerHTML='';const list=rows.length?rows:[{days:30,validFrom:''}];list.forEach((row,i)=>vacationEntitlementRows.appendChild(vacationRow(row,i)))}
  function collectPositionHistory(issues){const rows=[...positionHistoryRows.querySelectorAll('[data-history-row="position"]')].map((row,i)=>({position:(row.querySelector('[data-history-value="position"]')?.value||'').trim(),validFrom:row.querySelector('[data-history-date="position"]')?.value||'',i:i+1}));const out=[];for(const r of rows){if(r.position){if(!r.validFrom)issues.push(`• Position ${r.i}: Bitte „gültig ab“ angeben.`);else out.push({position:r.position,validFrom:r.validFrom});}else if(r.validFrom)issues.push(`• Position ${r.i}: Bitte die Tätigkeit/Position ergänzen.`)}if(!out.length)issues.push('• Bitte mindestens eine Position / Tätigkeit mit Gültigkeitsdatum angeben.');for(let i=1;i<out.length;i++)if(out[i].validFrom<=out[i-1].validFrom)issues.push('• Die Datumswerte des Karriereverlaufs müssen chronologisch aufsteigend sein.');return out}
  function collectVacationEntitlements(issues){const rows=[...vacationEntitlementRows.querySelectorAll('[data-history-row="vacation"]')],out=[];rows.forEach((row,i)=>{const raw=row.querySelector('[data-history-value="vacation"]')?.value??'',validFrom=i===0?'':(row.querySelector('[data-history-date="vacation"]')?.value||'');if(raw!==''&&raw!=null){const days=Number(raw);if(!Number.isFinite(days)||days<0)issues.push(`• Urlaubsanspruch Stufe ${i+1}: Bitte eine gültige Zahl eingeben.`);else if(i>0&&!validFrom)issues.push(`• Urlaubsanspruch Stufe ${i+1}: Bitte „gültig ab“ angeben.`);else out.push({days,validFrom})}else if(i>0&&validFrom)issues.push(`• Urlaubsanspruch Stufe ${i+1}: Bitte Urlaubstage ergänzen.`)});for(let i=1;i<out.length;i++)if(out[i].validFrom<=out[i-1].validFrom)issues.push('• Die Datumswerte der Urlaubsansprüche müssen chronologisch aufsteigend sein.');return out}
  form.querySelector('#add-position-history').addEventListener('click',()=>positionHistoryRows.appendChild(positionRow({},positionHistoryRows.children.length)));
  form.querySelector('#add-vacation-entitlement').addEventListener('click',()=>vacationEntitlementRows.appendChild(vacationRow({},vacationEntitlementRows.children.length)));
  form.querySelector('#add-work-schedule').addEventListener('click',()=>workScheduleRows.appendChild(workScheduleRow({model:'uniform',validFrom:'',weeklyHours:40,workDays:['1','2','3','4','5'],changeType:'change'},workScheduleRows.children.length)));
  renderPositionHistory();renderVacationEntitlements();renderWorkSchedules();
  function syncLoginField(){
    const username=form.elements.loginType.value==='username';
    form.querySelector('#employee-login-label').textContent=username?'Benutzername':'E-Mail-Adresse';
    form.elements.login.type=username?'text':'email';
    form.elements.login.placeholder=username?'ohne Leerzeichen oder @':'name@firma.de';
  }
  form.elements.loginType.addEventListener('change',syncLoginField);
  syncLoginField();
  function syncManagementPortalPermissions(){
    const access=form.elements.managementPortalAccess;
    const documentAccess=form.elements.managementPortalDocumentAccess;
    const option=form.querySelector('#management-portal-document-option');
    const enabled=access?.checked===true;
    if(documentAccess){
      documentAccess.disabled=!enabled;
      if(!enabled)documentAccess.checked=false;
    }
  ...
  function pdsFindArea(code){return businessAreas.find(a=>String(a.code||'')===String(code||''))||null}
  function pdsFindInsurer(text){const n=normText(String(text||'').replace(/^\d+\s*-\s*/,''));return insurers.find(x=>{const xn=normText(x.name);return n&&(n.includes(xn)||xn.includes(n)||String(x.code||'')&&String(text||'').includes(String(x.code)))})||null}
  function pdsFindBank(text,bic){const b=String(bic||'').replace(/\s/g,'').toUpperCase();if(b){const hit=banks.find(x=>String(x.bic||'').replace(/\s/g,'').toUpperCase()===b);if(hit)return hit}const n=normText(text);return banks.find(x=>n&&(n.includes(normText(x.name))||normText(x.name).includes(n)))||null}
  ...
  const cancel=()=>tab('show');el.querySelector('#cancel-user').onclick=cancel;el.querySelector('#cancel-user-bottom').onclick=cancel;
    ...
    if(!u)return;
    if(!await confirmDialog(`Mitarbeiter „${u.name||u.email||u.username}“ wirklich entfernen?\n\nDer Zugang wird deaktiviert und aus der aktiven Mitarbeiterübersicht entfernt. Zeit-, Schulungs-, Abrechnungs- und Historiendaten bleiben aus Nachweisgründen erhalten.`,{danger:true}))return;
    try{
...
const renderPhotoEditor=(u)=>{
  ...
  const input=box.querySelector('input[type=file]');if(input)input.onchange=async()=>{const file=input.files?.[0];if(!file)return;try{const result=await uploadEmployeePhoto(ctx,u.id,file);if(result.url)photoUrls[u.id]=result.url;toast('Mitarbeiterfoto gespeichert.');renderPhotoEditor(u)}catch(err){console.error(err);toast(err?.message||'Foto konnte nicht gespeichert werden.')}};
  const del=box.querySelector('.employee-photo-delete');if(del)del.onclick=async()=>{if(!await confirmDialog('Mitarbeiterfoto wirklich entfernen?'))return;try{await deleteEmployeePhoto(ctx,u.id);delete photoUrls[u.id];toast('Mitarbeiterfoto entfernt.');renderPhotoEditor(u)}catch(err){console.error(err);toast(err?.message||'Foto konnte nicht entfernt werden.')}};
};

el.querySelectorAll('.reset-password').forEach(b=>b.onclick=async()=>{
    const u=users.find(x=>x.id===b.dataset.id);
    if(!u||u.hasRealEmail!==false){toast('Passwort-Reset ist nur für Zugänge mit Benutzername vorgesehen.','error');return;}
    if(!canResetPassword){toast('Keine Berechtigung zum Zurücksetzen von Passwörtern.','error');return;}
    const password=await inputDialog(`Neues Startpasswort für ${u.name||u.username||'Mitarbeiter'} vergeben:`,'',{title:'Passwort zurücksetzen',inputType:'text',placeholder:'mind. 8 Zeichen inkl. Groß/Klein, Zahl, Sonderzeichen'});
    if(password===null)return;
    const value=String(password||'');
    if(!isStrongPassword(value)){toast('Mindestens 8 Zeichen, Groß- und Kleinbuchstaben, mindestens 1 Zahl und 1 Sonderzeichen erforderlich.','error');return;}
    const confirmPassword=await inputDialog('Startpasswort zur Sicherheit erneut eingeben:','',{title:'Passwort bestätigen',inputType:'text',placeholder:'Startpasswort wiederholen'});
    if(confirmPassword===null)return;
    if(String(confirmPassword)!==value){toast('Die beiden Passwörter stimmen nicht überein.','error');return;}
    if(!await confirmDialog(`Passwort für „${u.username||u.name||'Mitarbeiter'}“ wirklich zurücksetzen? Das bisherige Passwort ist danach sofort ungültig.`))return;
    const oldText=b.textContent;b.disabled=true;b.textContent='Wird zurückgesetzt …';
    try{
      const idToken=await ctx.user.getIdToken();
      await resetUsernamePassword({idToken,targetUserId:u.id,newPassword:value});
      await addDoc(collection(db,'employeeHistory'),historyRecord(ctx,u.id,u,'password_reset',[]));
      toast('Passwort wurde zurückgesetzt. Das neue Startpasswort kann jetzt an den Mitarbeiter übergeben werden.');
    }catch(err){
      console.error(err);
      const msg=String(err?.message||'');
      toast(msg.includes('Berechtigung')?'Keine Berechtigung zum Zurücksetzen von Passwörtern.':(err?.message||'Passwort konnte nicht zurückgesetzt werden.'),'error');
  ...
  el.querySelectorAll('.edit-user').forEach(b=>b.onclick=async()=>{
    ...
    ['birthDate','salutation','title','firstName','lastName','gender','birthName','birthPlace','birthNationality','healthInsuranceNumber','maritalStatus','marriageDate','street','postalCode','city','privateEmail','phone','mobile','emergencyContactName','emergencyContactPhone','taxId','taxClass','religion','socialSecurityNumber','healthInsuranceId','insuranceType','personGroup','contributionGroup','iban','bic','bankId','accountHolder','salaryValidFrom'].forEach(k=>setVal(form,k,p[k]||''));setVal(form,'childAllowance',p.childAllowance??'');setVal(form,'grossSalary',p.grossSalary??'');setVal(form,'hourlyRate',p.hourlyRate??'');boolVal(form,'birthdayList',p.birthdayList===true);
    ...
    const loadedSupervisorPermissions=normalizedSupervisorPermissions(u);form.querySelectorAll('[name=supervisorPermission]').forEach(x=>x.checked=loadedSupervisorPermissions[x.value]!==false);
    const syncRolePermissionVisibility=()=>{const adminBox=form.querySelector('#admin-permissions-box'),supervisorBox=form.querySelector('#supervisor-permissions-box');if(adminBox)adminBox.classList.toggle('hidden',form.elements.role.value!=='admin');if(supervisorBox)supervisorBox.classList.toggle('hidden',form.elements.role.value!=='supervisor')};syncRolePermissionVisibility();syncAdminRoleSections();
  ...
  form.onsubmit=async e=>{
    ...
    if((formMode==='new'&&id)||(formMode==='edit'&&(!id||id!==editingUserId))){toast('Der Formularzustand ist nicht eindeutig. Bitte den Bereich Mitarbeiter neu öffnen und die Eingabe erneut beginnen. Es wurde nichts gespeichert.','error');return;}
    ...
    const primarySupervisor=f.elements.supervisorId.value||'',secondarySupervisor=f.elements.supervisorId2.value||'';
    const areaSelection=f.elements.businessAreaId.value,selectedArea=businessAreaMap.get(areaSelection),legacyArea=areaSelection.startsWith('legacy:')?areaSelection.slice(7):'';
    const employeeNumber=f.elements.employeeNumber.value.trim(),companyAreaNumber=selectedArea?.code||legacyArea;
    const issues=[];
    if(secondarySupervisor&&!primarySupervisor)issues.push('• Bitte zuerst einen 1. Vorgesetzten auswählen.');
    if(primarySupervisor&&secondarySupervisor&&primarySupervisor===secondarySupervisor)issues.push('• 1. und 2. Vorgesetzter müssen unterschiedlich sein.');
    if(id&&(primarySupervisor===id||secondarySupervisor===id))issues.push('• Ein Mitarbeiter kann nicht sein eigener Vorgesetzter sein.');
    if(!/^\d{5}$/.test(employeeNumber))issues.push('• Die Mitarbeiternummer muss fünfstellig numerisch sein.');
    if(!/^\d{3}$/.test(companyAreaNumber))issues.push('• Bitte einen gültigen dreistelligen Geschäftsbereich auswählen.');
    if(!id){
      if(loginType==='username'&&!/^[a-z0-9._+-]+$/i.test(login))issues.push('• Benutzername: Nur Buchstaben, Ziffern, Punkt, Unterstrich, Plus oder Bindestrich verwenden; keine Leerzeichen oder @.');
      if(loginType==='email'&&!f.elements.login.validity.valid)issues.push('• Bitte eine gültige E-Mail-Adresse eingeben.');
      if(!isStrongPassword(password))issues.push('• Startpasswort: mindestens 8 Zeichen, Groß- und Kleinbuchstaben, mindestens 1 Zahl und 1 Sonderzeichen.');
    }
    const vacationEntitlements=collectVacationEntitlements(issues);
    const positionHistory=collectPositionHistory(issues);
    const workScheduleHistory=collectWorkSchedules(issues);
    const requestedPause={active:f.elements.specialPauseActive.checked===true,minutes:Math.round(Number(f.elements.specialPauseMinutes.value)||0),validFrom:f.elements.specialPauseValidFrom.value||''};
    if(requestedPause.active&&requestedPause.minutes<30)issues.push('• Die besondere Pause muss mindestens 30 Minuten betragen.');
    const oldPauseRows=Array.isArray(previousUser?.pauseRuleHistory)?previousUser.pauseRuleHistory.filter(x=>x?.validFrom).sort((a,b)=>String(a.validFrom).localeCompare(String(b.validFrom))):[],oldPause=oldPauseRows[oldPauseRows.length-1]||null,pauseChanged=requestedPause.active!==(oldPause?.active===true)||(requestedPause.active&&requestedPause.minutes!==Math.round(Number(oldPause?.minutes)||30));
    if(pauseChanged&&!requestedPause.validFrom)issues.push('• Bitte für die geänderte besondere Pausenregelung „gültig ab“ angeben.');
    if(requestedPause.validFrom&&oldPauseRows.some(x=>x.validFrom===requestedPause.validFrom))issues.push('• Für dieses Gültigkeitsdatum existiert bereits eine Pausenregelung.');
    if(id&&previousUser&& !sameTrainingSelection(previousUser.bereiche,previousUser.extraTrainings,[...f.querySelectorAll('[name=bereich]:checked')].map(x=>x.value),[...f.querySelectorAll('[name=extraTraining]:checked')].map(x=>x.value))&&!f.elements.trainingValidFrom.value)issues.push('• Das Datum „gültig ab“ für die Schulungszuordnung fehlt.');
    if(issues.length){validationNotice(['Bitte folgende Angaben prüfen:',...issues]);return;}
    const activeSchedule=workScheduleOn({workScheduleHistory},new Date().toISOString().slice(0,10));
    const pauseRuleHistory=[...oldPauseRows];if(pauseChanged)pauseRuleHistory.push({active:requestedPause.active,minutes:requestedPause.active?requestedPause.minutes:Math.max(30,requestedPause.minutes||30),validFrom:requestedPause.validFrom});
    const publicData={name:f.elements.name.value.trim(),companyId:f.elements.companyId.value,email,username:loginType==='username'?login.toLowerCase():'',hasRealEmail:loginType==='email',role:f.elements.role.value,managementPortalAccess:f.elements.managementPortalAccess.checked===true,managementPortalDocumentAccess:f.elements.managementPortalAccess.checked===true&&f.elements.managementPortalDocumentAccess.checked===true,adminPermissions:f.elements.role.value==='admin'?(canManagePermissions?Object.fromEntries(ADMIN_PERMISSION_DEFS.map(x=>[x.key,[...f.querySelectorAll('[name=adminPermission]')].find(c=>c.value===x.key)?.checked===true])):(previousUser?.adminPermissions||DEFAULT_ADMIN_PERMISSIONS)):{},supervisorPermissions:f.elements.role.value==='supervisor'?(canManagePermissions?Object.fromEntries(SUPERVISOR_PERMISSION_DEFS.map(x=>[x.key,[...f.querySelectorAll('[name=supervisorPermission]')].find(c=>c.value===x.key)?.checked===true])):(previousUser?.supervisorPermissions||{})):{},supervisorId:f.elements.supervisorId.value||null,supervisorId2:f.elements.supervisorId2.value||null,active:f.elements.active.value==='true',startDate:f.elements.startDate.value||null,endDate:f.elements.endDate.value||null,weeklyHours:activeSchedule.weeklyHours,workScheduleHistory,pauseRuleHistory,vacationDays:Number(f.elements.vacationDays.value||30),vacationEntitlements,earliestStartTime:f.elements.earliestStartTime.value||'',employeeNumber,businessAreaId:selectedArea?.id||'',companyAreaNumber,projectTimeTracking:f.elements.projectTimeTracking.value==='true',flatEightHourEmployeeView:f.elements.flatEightHourEmployeeView.checked===true,noTimeTracking:f.elements.noTimeTracking.checked===true,department:f.elements.department.value.trim(),position:positionHistory[0]?.position||'',positionHistory,contractType:f.elements.contractType.value,probationEndDate:f.elements.probationEndDate.value||null,fixedTermEndDate:f.elements.fixedTermEndDate.value||null,costCenter:f.elements.costCenter.value.trim(),workDays:activeSchedule.workDays,firstAider:f.elements.firstAider.value==='true',firstAiderValidUntil:f.elements.firstAiderValidUntil.value||null,fireWarden:f.elements.fireWarden.value==='true',fireWardenValidUntil:f.elements.fireWardenValidUntil.value||null,forkliftPermit:f.elements.forkliftPermit.value==='true',forkliftPermitValidUntil:f.elements.forkliftPermitValidUntil.value||null,aerialLiftPermit:f.elements.aerialLiftPermit.value==='true',aerialLiftPermitValidUntil:f.elements.aerialLiftPermitValidUntil.value||null,drivingLicenseClasses:f.elements.drivingLicenseClasses.value.trim(),nextDrivingLicenseCheck:f.elements.nextDrivingLicenseCheck.value||null,occupationalMedicalNotes:f.elements.occupationalMedicalNotes.value.trim(),bereiche:[...f.querySelectorAll('[name=bereich]:checked')].map(x=>x.value),extraTrainings:[...f.querySelectorAll('[name=extraTraining]:checked')].map(x=>x.value),updatedAt:serverTimestamp()};
    publicData.vacationDays=vacationEntitlementOn(publicData);publicData.position=positionOn(publicData);
    if(id&&previousUser){const before=normalizedWorkScheduleHistory(previousUser).map(({order,legacy,...row})=>row),after=normalizedWorkScheduleHistory(publicData).map(({order,legacy,...row})=>row),scheduleChanged=JSON.stringify(before)!==JSON.stringify(after),legacyDays=[...(previousUser.workDays?.length?previousUser.workDays:['1','2','3','4','5'])].map(String).sort(),first=workScheduleHistory[0],pureLegacyMigration=!Array.isArray(previousUser.workScheduleHistory)&&workScheduleHistory.length===1&&first?.model==='uniform'&&Number(first.weeklyHours)===Number(previousUser.weeklyHours??40)&&JSON.stringify([...first.workDays].sort())===JSON.stringify(legacyDays);const today=new Date().toISOString().slice(0,10),retroactive=workScheduleHistory.some(row=>row.validFrom<=today);if(scheduleChanged&&!pureLegacyMigration&&retroactive&&!await confirmDialog('Die Arbeitszeitregelung enthält eine heute oder rückwirkend gültige Änderung. Dadurch werden Soll-Zeiten und Stundenstände für betroffene Tage beim nächsten Laden neu berechnet. Die Änderung wird in der Mitarbeiterhistorie protokolliert. Wirklich speichern?'))return;}
    ...
    try{
      ...
      if(id){
        ...
        let authHandle=null,firestoreCommitted=false;
        try{
          authHandle=await createAuthAccount(email,password);
          const uid=authHandle.uid;
          batch.set(doc(db,'users',uid),{...publicData,email,mustChangePassword:true,createdAt:serverTimestamp()});
          ...
          if(authHandle&&!firestoreCommitted)await closeSecondaryAuth(authHandle,{rollback:true});
          throw createErr;
        }
      }
