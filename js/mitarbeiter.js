...
export async function renderMitarbeiter(el,ctx){
  ...
  const personalBody=`
    <label class="field"><span>Name (Anzeige)</span><input name="name" required></label><div class="field birthday-date-field"><span>Geburtsdatum</span><div class="birthday-date-line"><input name="birthDate" type="date"><label class="inline-check compact"><input name="birthdayList" type="checkbox"><span>Geburtstagsliste ja</span></label></div><small>Nur bei Zustimmung wird der Geburtstag in der Geburtstagsliste ausgegeben.</small></div>
  ...
  function applyPdsImport(resolved){
    ...
    if(resolved.parsed.businessEmail){setVal(form,'loginType','email');setVal(form,'login',resolved.parsed.businessEmail)}
