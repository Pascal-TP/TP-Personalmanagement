...
import { normalizedVacationEntitlements, vacationEntitlementOn, normalizedPositionHistory, positionOn } from "./employment-utils.js";
...
export async function renderMitarbeiter(el,ctx){
  ...
  const positionHistoryRows=form.querySelector('#position-history-rows'),vacationEntitlementRows=form.querySelector('#vacation-entitlement-rows');
  ...
  renderPositionHistory();renderVacationEntitlements();
  ...
  function applyPdsImport(resolved){
    ...
    if(Array.isArray(resolved.parsed.workDays)&&resolved.parsed.workDays.length){form.querySelectorAll('[name=workDay]').forEach(x=>x.checked=resolved.parsed.workDays.includes(x.value))}
