...
export async function renderMitarbeiter(el,ctx){
  ...
  el.querySelectorAll('.edit-user').forEach(b=>b.onclick=async()=>{
    ...
    el.querySelector('#employee-form-title').textContent=`Mitarbeiterkartei · ${u.name||'Mitarbeiter'}`;tab('create');await renderPersonalakte(akte,ctx,u);window.scrollTo({top:0,behavior:'smooth'});
  });

  form.onsubmit=async e=>{
    e.preventDefault();
    const f=e.currentTarget,id=f.elements.id.value,loginType=f.elements.loginType.value,login=f.elements.login.value.trim(),password=f.elements.password.value,email=loginType==='username'?syntheticEmail(login):login.toLowerCase();
