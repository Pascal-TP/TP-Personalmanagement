...
export async function renderMitarbeiter(el,ctx){
  ...
  const show=el.querySelector('#user-show'),create=el.querySelector('#user-create'),form=el.querySelector('#user-form'),akte=el.querySelector('#personalakte-container'),pdsImportBtn=el.querySelector('#pds-pdf-import'),pdsFile=el.querySelector('#pds-pdf-file'),pdsDialog=el.querySelector('#pds-import-dialog');
  const PDS_FIELD_LABELS={name:'Name',employeeNumber:'Mitarbeiternummer',birthDate:'Geburtsdatum',street:'Straße / Hausnummer',postalCode:'PLZ',city:'Ort',privateEmail:'Private E-Mail',mobile:'Mobil',emergencyContactName:'Notfallkontakt',emergencyContactPhone:'Telefon Notfallkontakt',companyId:'Firma',businessAreaId:'Geschäftsbereich',department:'Abteilung',position:'Position / Tätigkeit',costCenter:'Kostenstelle',contractType:'Beschäftigungsart',startDate:'Eintritt',endDate:'Austritt',weeklyHours:'Wochenstunden',vacationDays:'Urlaubstage/Jahr',taxId:'Steuer-ID',taxClass:'Steuerklasse',childAllowance:'Kinderfreibetrag',socialSecurityNumber:'Sozialversicherungsnummer',healthInsuranceId:'Krankenkasse',insuranceType:'Versicherungsart',personGroup:'Personengruppe',contributionGroup:'Beitragsgruppe',iban:'IBAN',bic:'BIC',bankId:'Bank',accountHolder:'Kontoinhaber',grossSalary:'Bruttogehalt / Monat (€)',salaryValidFrom:'Gültig ab',login:'E-Mail / Benutzername'};
  ...
  function pdsResolve(parsed){
    const company=pdsFindCompany(parsed.companyText),area=pdsFindArea(parsed.businessAreaCode),insurer=pdsFindInsurer(parsed.healthInsuranceText),bank=pdsFindBank(parsed.bankText,parsed.bic);
    const values={name:parsed.name||'',employeeNumber:parsed.employeeNumber||'',birthDate:parsed.birthDate||'',street:parsed.street||'',postalCode:parsed.postalCode||'',city:parsed.city||'',privateEmail:parsed.privateEmail||'',mobile:parsed.mobile||'',emergencyContactName:parsed.emergencyContactName||'',emergencyContactPhone:parsed.emergencyContactPhone||'',companyId:company?.id||'',businessAreaId:area?.id||'',department:parsed.department||'',position:parsed.position||'',costCenter:parsed.costCenter||'',contractType:parsed.contractType||'',startDate:parsed.startDate||'',endDate:parsed.endDate||'',weeklyHours:parsed.weeklyHours??'',vacationDays:parsed.vacationDays??'',taxId:parsed.taxId||'',taxClass:parsed.taxClass||'',childAllowance:parsed.childAllowance??'',socialSecurityNumber:parsed.socialSecurityNumber||'',healthInsuranceId:insurer?.id||'',insuranceType:parsed.insuranceType||'',personGroup:parsed.personGroup||'',contributionGroup:parsed.contributionGroup||'',iban:parsed.iban||'',bic:parsed.bic||'',bankId:bank?.id||'',accountHolder:parsed.accountHolder||'',grossSalary:parsed.grossSalary??'',salaryValidFrom:parsed.salaryValidFrom||'',login:parsed.businessEmail||''};
    return {parsed,values,company,area,insurer,bank};
  }
  ...
  function pdsDisplayValue(key,value,resolved){if(key==='companyId')return resolved.company?.name||resolved.parsed.companyText||'nicht zugeordnet';if(key==='businessAreaId')return resolved.area?`${resolved.area.code} · ${resolved.area.name}`:(resolved.parsed.businessAreaCode?`${resolved.parsed.businessAreaCode} · nicht in Stammdaten gefunden`:'');if(key==='healthInsuranceId')return resolved.insurer?.name||resolved.parsed.healthInsuranceText||'nicht zugeordnet';if(key==='bankId')return resolved.bank?.name||resolved.parsed.bankText||'nicht zugeordnet';if(key==='workDays')return Array.isArray(value)?value.join(', '):value;return value}
  function showPdsPreview(resolved){
    ...
    if(resolved.parsed.businessAreaCode&&!resolved.area)mappingWarnings.push(`Geschäftsbereich/Abteilung ${resolved.parsed.businessAreaCode} wurde in den TP-Stammdaten nicht gefunden.`);
    if(resolved.parsed.healthInsuranceText&&!resolved.insurer)mappingWarnings.push(`Krankenkasse „${resolved.parsed.healthInsuranceText}“ wurde nicht in den Stammdaten gefunden.`);
    if(resolved.parsed.bankText&&!resolved.bank)mappingWarnings.push(`Bank „${resolved.parsed.bankText}“ wurde nicht in den Stammdaten gefunden.`);
    ...
    const rows=Object.entries(resolved.values).filter(([,v])=>v!==''&&v!==null&&v!==undefined).map(([key,value])=>`<tr><th>${esc(PDS_FIELD_LABELS[key]||key)}</th><td>${esc(pdsDisplayValue(key,value,resolved))}</td></tr>`).join('');
