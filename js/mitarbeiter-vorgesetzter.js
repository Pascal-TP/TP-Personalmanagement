...
function bookingSourceLabel(record){
  ...
  if(record.source==='supervisor_adjustment')return 'Vorgesetzten-Korrektur';
...
async function renderSupervisorBookings(container,employee){
  ...
  try{
    ...
    container.innerHTML='<div class="error-card compact">Die Zeitbuchungen konnten nicht geladen werden.</div>';
    return;
  }
  ...
  const renderMonth=()=>{
    ...
    const monthRecords=records.filter(r=>bookingDateKey(r).startsWith(`${year}-${bookingPad(monthNo)}-`)).sort((a,b)=>{
    ...
    container.innerHTML=`
      ...
      <div class="table-wrap"><table class="employee-bookings-table">
        ...
        <tbody>
          ${monthRecords.length?monthRecords.map(r=>{
            ...
            const terminal=[
              ...
              r.terminalEndId&&r.terminalEndId!==(r.terminalId||'')?`GEHEN: ${r.terminalEndId}`:''
            ].filter(Boolean).join(' · ');
            return `<tr>
          ...
          }).join(''):`<tr><td colspan="7" class="empty">Für ${esc(bookingMonthLabel(month))} sind keine Buchungen vorhanden.</td></tr>`}
        </tbody>
      </table></div>`;

    container.querySelector('[data-booking-nav="prev"]').onclick=()=>{month=new Date(month.getFullYear(),month.getMonth()-1,1,12);renderMonth()};
    container.querySelector('[data-booking-nav="current"]').onclick=()=>{const now=new Date();month=new Date(now.getFullYear(),now.getMonth(),1,12);renderMonth()};
    container.querySelector('[data-booking-nav="next"]').onclick=()=>{month=new Date(month.getFullYear(),month.getMonth()+1,1,12);renderMonth()};
  };
...
export async function renderSupervisorMitarbeiter(el,ctx){
  ...
  el.querySelectorAll('.supervisor-employee-card').forEach(button=>button.onclick=async()=>{
    ...
    try{
      ...
      const bookingBox=detail.querySelector('#supervisor-employee-bookings');
